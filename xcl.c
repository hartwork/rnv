...
static void flush_text(void) {
  current=rnv_flush_text(previous=current,text,n_t,mixed);
  text[n_t=0]='\0';
}
...
static void start_element(void *userData,const char *name,const char **attrs) {
  if(current!=rn_notAllowed) { 
    mixed=1;
    flush_text();
    current=rnv_start_element(previous=current,(char*)name,(char**)attrs);
    mixed=0;
  } else {
    ++level;
  }
...
static void end_element(void *userData,const char *name) {
  if(current!=rn_notAllowed) {
    flush_text(); 
    current=rnv_end_element(previous=current,(char*)name);
    mixed=1;
  } else {
